...
class NimAI():
    ...
    def get_q_value(self, state, action):
        ...
        return q_value if q_value else 0
    ...
    def best_future_reward(self, state):
        ...
        best_action = available_actions.pop()
        best_q = self.get_q_value(state, best_action)
        for action in available_actions:
            actions_q = self.get_q_value(state, action)
            if (actions_q > best_q):
                best_action = action
                best_q = actions_q
    ...
    def choose_action(self, state, epsilon=True):
        ...
        available_actions = Nim.available_actions(state)
        if len(available_actions) == 0: return 0
        # Check to see if should take a random action
        if (epsilon):
            roll_value = random.uniform(0, 1)
            if roll_value < self.epsilon:
