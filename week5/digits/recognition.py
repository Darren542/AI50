    ...
    for i in range(ROWS):
        ...
        for j in range(COLS):
            ...
            if mouse and rect.collidepoint(mouse):
                handwriting[i][j] = max(handwriting[i][j], 250 / 255)
                if i + 1 < ROWS:
                    handwriting[i + 1][j] = max(handwriting[i + 1][j], 220 / 255)
                if j + 1 < COLS:
                    handwriting[i][j + 1] = max(handwriting[i][j + 1], 220 / 255)
                if i + 1 < ROWS and j + 1 < COLS:
                    handwriting[i + 1][j + 1] = max(handwriting[i + 1][j + 1], 190 / 255)
